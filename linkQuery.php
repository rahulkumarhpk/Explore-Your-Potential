...
<?php
		...
		$num=1;
		while($num<=$_SESSION["queryCount"]){
			

			if (isset($_POST["dismiss$num"])) { // DISMISSING QUERY

				$conn = mysqli_connect($host, $user, $pwd);
				if ($conn) {
					if (mysqli_select_db($conn, $dbname)) {

						$queryToUnlink = $_POST["queryInAction$num"];

						$sqlb = "UPDATE `QUERIES` SET `query_status`=1 WHERE `s_no`= $queryToUnlink";

						if(mysqli_query($conn, $sqlb)){
							echo '<script>
										alert("Query dismissed");
										window.location.href="counselling.php";
								</script>';
						}
						else
							echo '<script>
										alert("Unable to dismiss query");
										window.location.href="counselling.php";
								</script>';
					}
					else
			...
			if (isset($_POST["view$num"])) { // VIEWING REPORT
				...
				if ($conn) {
					if (mysqli_select_db($conn, $dbname)) {
						...
						$reportToView = $_POST["userInAction$num"];
						...
						if(mysqli_num_rows($resulta)>0){
							...
							echo '<script>
										alert("Unable to view report. Perhaps this user hasn\'t taken the test yet!");
										window.location.href="counselling.php";
								</script>';
					}
					else
